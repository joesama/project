<div class="col-md-12" style="padding: 0px 5px">
  <div class="panel">
    ...
    @php
      ...
      $projectInfoId = $infoProject->id;
      $reportStart = '';
      $reportEnd = '';
      $reportDue = '';
      $last_status = '';
      $action = route('api.workflow.info',[$corporateId, $projectId, $projectInfoId]);
      $firstStep = $infoWorkflow->pluck('step')->first();
      $sliceIndex = $infoWorkflow->pluck('step')->search(data_get($infoProject,'need_step'));
      $next = $infoWorkflow->slice($sliceIndex+1,1)->first();
      $back = $infoWorkflow->first();

      $currentAction = $infoWorkflow->where('profile.user_id',auth()->id())
                      // ->where("approval",null)
                      ->where("step",data_get($infoProject,'need_step'))
                      ->first();
      $allTrails = data_get($infoProject,'workflow');
      $profileRole = data_get($currentAction,'project.profile.pivot.role_id');
    @endphp
        ...
        @if( $currentAction )
          ...
          'back_action' => ($firstStep != data_get($infoProject,'need_step') && $profileRole != 2) ? data_get($back,'profile.id') : FALSE,
